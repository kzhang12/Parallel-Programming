...
class SealedDES implements Runnable
{
    ...
    public SealedDES (long start, long end, int num)
    {
	sta = start;
	en = end;
	n = num;
...
if ( 2 != args.length )
{
System.out.println ("Usage: java SealedDES number_threads key_size_in_bits");
return;
}
...
ArrayList<SealedDES> deccipher = new ArrayList<SealedDES>();

// Create a simple cipher
for (int i = 0; i < num_threads; i++) {
    SealedDES d = new SealedDES (maxk/num_threads*i,maxk/num_threads * (i+1),i);
    deccipher.add(d);
...
for (int i = 0; i < num_threads; i++) {
    ...
    Thread e = new Thread(deccipher.get(i));
    t.add(e);
...
System.out.println ( "Completed search of " + keys + " keys at " + elapsed + " milliseconds.");
	...
	for ( long i = sta; i < en; i++ ) {
	    this.setKey ( i );
	    String decryptstr = this.decrypt ( sldOb );
		...
		{
		    long elapsed = System.currentTimeMillis() - runstart;
		    //System.out.println ( "Thread " + this.returnNum() + " Searched key number " + i + " at " + elapsed + " milliseconds.");
		}
	}
